...
class GraffitiReport(Base):
    ...
    user_id = Column(BigInteger, ForeignKey("users.user_id", ondelete="CASCADE"), nullable=True)  # Ссылка на пользователя
    ...
    __table_args__ = (
        ...
        Index('idx_graffiti_reports_location', 'location', postgresql_using='gist'),  # Индекс для быстрого геопоиска (ST_DWithin)
    )
...
class ReportPhoto(Base):
    ...
    report_id = Column(Integer, ForeignKey("graffiti_reports.report_id", ondelete="CASCADE"), nullable=False)  # Ссылка на заявку
